class Notification < ApplicationRecord
  ...
  def notification_info current_user
    if notice_type == "react"
      react = React.find(notification_with_id)
      if react.micropost.micropost_id.nil?
        #post
        return "/microposts/#{react.micropost.id}?notif_id=#{id}&locale=#{current_user.locale}" #href Oauth 2- auto redirect
      else
        #comment
        position = Micropost.where("micropost_id = ? AND created_at >= ?", react.micropost.micropost_id, react.micropost.created_at).count
        return "/microposts/#{react.micropost.micropost_id}?page=#{position}&per_page=1&notif_id=#{id}&locale=#{current_user.locale}"
      end
    end
    ...
    if notice_type == "comment"
      ...
      if !comment.micropost_id.nil?
        ...
        return "/microposts/#{comment.micropost_id}?page=#{position}&per_page=1&notif_id=#{id}&locale=#{current_user.locale}"
      end
    end
  end
end
